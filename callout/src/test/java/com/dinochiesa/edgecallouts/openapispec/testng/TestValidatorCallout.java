package com.dinochiesa.edgecallouts.openapispec.testng;
...
import com.dinochiesa.edgecallouts.openapispec.ValidatorCallout;
...
public class TestValidatorCallout {
            ...
            if (name.startsWith("test") && name.endsWith(".json")) {
                ...
                for (String key : tc.getContext().keySet()) {
                    ...
                    if (value.startsWith("file://")) {
                        String name2 = value.substring(7);
                        Path path = Paths.get(testDataDir, name2);
                        if (!Files.exists(path)) {
                            throw new IOException(String.format("test(%s): file(%s) for context(%s) not found",
                                                                name, name2, key));
                        }
    ...
    public void test2_Configs(TestCase tc) {
        ...
        if (expectedResult != actualResult) {
            ...
            String actualError = msgCtxt.getVariable("oas_error");
            ...
            if (StringUtils.isNotBlank(expectedError)) {
                ...
                boolean actualValid = msgCtxt.getVariable("oas_valid");
